...
private:
	...
	void destructor_aux(BTreeNode<K, I>* root);
public:
	BTree(): root(NULL) {}
	...
	~BTree();
private:
	int node_count_aux(BTreeNode<K, I>* root);
	int leaves_count_aux(BTreeNode<K, I>* root);
	void swap_subtrees_aux(BTreeNode<K, I>* root);
};

template <class K, class I>
void BTree<K, I>::destructor_aux(BTreeNode<K, I>* root) {
	if (root == NULL) {
		return;
	}
	else {
		destructor_aux(root->left);
		destructor_aux(root->right);
		delete root;
		return;
	}
}
...
template <class K, class I>
int BTree<K, I>::node_count_aux(BTreeNode<K, I>* root) {
	if (root == NULL) {
		return 0;
	}
	else {
		int left = node_count_aux(root->left);
		int right = node_count_aux(root->right);
		return left + right + 1;
	}
}

template <class K, class I>
int BTree<K, I>::node_count() {
	return node_count_aux(root);
}

template <class K, class I>
int BTree<K, I>::leaves_count_aux(BTreeNode<K, I>* root) {
	if (root == NULL) {
		return 0;
	}
		...
		int left = leaves_count_aux(root->left);
		int right = leaves_count_aux(root->right);
		return left + right;
	}
}

template <class K, class I>
int BTree<K, I>::leaves_count() {
	return leaves_count_aux(root);
}

template <class K, class I>
void BTree<K, I>::swap_subtrees_aux(BTreeNode<K, I>* root) {
	// THIS FUNCTION BREAKS THE BINARY SEARCH TREE INVARIANT
	if (root == NULL) {
		return;
	}
	else {
		std::swap(root->left, root->right);
		swap_subtrees_aux(root->left);
		swap_subtrees_aux(root->right);
	}
}

template <class K, class I>
void BTree<K, I>::swap_subtrees() {
	std::cout << "WARNING: EXECUTING THIS FUNCTION BREAKS THE BINARY TREE INVARIANT!\n";
	swap_subtrees_aux(root);
}
#endif